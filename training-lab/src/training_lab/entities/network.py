...
class TarokNet(nn.Module):
    ...
    def _extract_card_features(self, state: torch.Tensor) -> torch.Tensor:
        ...
        played = state[:, 54:108]
        trick = state[:, 108:162]
        belief_start = OLD_STATE_SIZE_V1  # 270
    ...
    def load_state_dict(self, state_dict, strict=True, assign=False):
        ...
        # If loading into oracle model but checkpoint lacks oracle keys, relax strict
        if self.oracle_critic_enabled and not any(k.startswith("critic_backbone.") for k in state_dict):
            strict = False
    ...
    def get_actor_features(self, state: torch.Tensor) -> torch.Tensor:
    ...
    def get_critic_features(self, oracle_state: torch.Tensor) -> torch.Tensor:
