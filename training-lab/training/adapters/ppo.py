...
class PPOAdapter(PPOPort):
    def __init__(self) -> None:
        ...
        # PPO hyperparams — overridden from config in setup()
    ...
    def setup(self, weights: dict, config: TrainingConfig, device: str) -> None:
        ...
        self._imitation_coef = float(getattr(config, "imitation_coef", 0.0))
        self._gamma = float(getattr(config, "gamma", 0.99))
        self._gae_lambda = float(getattr(config, "gae_lambda", 0.95))
        self._clip_epsilon = float(getattr(config, "clip_epsilon", 0.2))
        self._value_coef = float(getattr(config, "value_coef", 0.5))
        self._entropy_coef = float(getattr(config, "entropy_coef", 0.01))
    ...
    def update(self, raw_experiences: dict[str, Any], nn_seats: list[int]) -> tuple[dict[str, float], dict]:
        ...
        # Only learn from the learner's own experiences (seats labelled "nn").
        # Frozen snapshots and heuristic bots produce log_prob/value from a
        # different model, so including them would corrupt the PPO ratio.
        raw_experiences = _keep_seats(raw_experiences, kept_seats=set(nn_seats))
        del nn_seats
        prepped = _prepare_batched(raw_experiences, gamma=self._gamma, gae_lambda=self._gae_lambda)
        metrics = self._ppo_update_batched(prepped)

        new_weights = {k: v.cpu() for k, v in self._network.state_dict().items()}
        # Ensure large intermediate tensors are dropped before returning.
        del prepped
...
def _keep_seats(raw: dict[str, Any], kept_seats: set[int]) -> dict[str, Any]:
    """Return a copy of *raw* keeping only experiences from *kept_seats*."""
    players_np = np.asarray(raw["players"])
    n_exp = len(players_np)
    keep = np.zeros(n_exp, dtype=bool)
    for s in kept_seats:
        keep |= players_np == s
    if keep.all():
        return raw
    ...
    for key, val in raw.items():
        arr = np.asarray(val)
        if arr.ndim == 0 or arr.shape[0] != n_exp:
            # Scalars or game-level arrays (scores, contracts, etc.) — keep as-is
            filtered[key] = val
        else:
...
def _prepare_batched(raw: dict[str, Any], gamma: float = 0.99, gae_lambda: float = 0.95) -> dict[str, Any]:
