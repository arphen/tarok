...
class PPOAdapter(PPOPort):
    ...
    def update(self, raw_experiences: dict[str, Any], nn_seats: list[int], bot_seats: list[int]) -> tuple[dict[str, float], dict]:
        ...
        # Only learn from NN seats — drop experiences generated by heuristic bots.
        if bot_seats:
            raw_experiences = _filter_seats(raw_experiences, excluded_seats=set(bot_seats))
...
def _filter_seats(raw: dict[str, Any], excluded_seats: set[int]) -> dict[str, Any]:
    """Return a copy of *raw* with all experiences from *excluded_seats* removed."""
    players_np = np.asarray(raw["players"])
    n_exp = len(players_np)
    keep = np.ones(n_exp, dtype=bool)
    for s in excluded_seats:
        keep &= players_np != s
    if keep.all():
        return raw
