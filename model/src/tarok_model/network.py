...
class TarokNet(nn.Module):
    ...
    def __init__(self, hidden_size: int = 256, oracle_critic: bool = False):
        ...
        # Each card has 9 per-plane features (v10): own hand, 3 belief
        # planes, own played, 3 opp played, active trick.
    ...
    def _extract_card_features(self, state: torch.Tensor) -> torch.Tensor:
        ...
        Returns (batch, 54, 9) tensor (v10 layout, all card planes 54-aligned):
...
class TarokNetV4(TarokNet):
    """v4 actor-critic: card-play head split by game mode family.
    ...
    def _mode_from_contract_idx(contract_idx: int) -> GameMode:
        if contract_idx in TarokNetV4._SOLO_CONTRACT_INDICES:
            return GameMode.SOLO
        if contract_idx in TarokNetV4._KLOP_BERAC_CONTRACT_INDICES:
            return GameMode.KLOP_BERAC
        if contract_idx == TarokNetV4._BARVNI_VALAT_IDX:
    ...
    def _resolve_modes(
        ...
        if game_mode is None:
            raise ValueError("TarokNetV4 requires explicit game_mode for card-play routing")
