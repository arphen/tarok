...
selection.  This lets DEAP evolve *how* an agent plays (aggressive bidder,
cautious defender, bold announcer) independently from the learned weights.
...
# Logit bias scale factor — how much a trait value of 1.0 shifts logits
_BIAS_SCALE = 1.5
...
class BehavioralProfile:
    ...
    # --- Bidding traits ---
    bid_aggression: float = 0.0     # Eagerness to bid (all non-pass bids)
    solo_propensity: float = 0.0    # Extra bias for solo contracts

    # --- Card play traits ---
    trump_eagerness: float = 0.0    # Bias toward playing taroks
    defensive_caution: float = 0.0  # When defending, bias toward low-value cards

    # --- Announcement traits ---
    announce_boldness: float = 0.0  # Willingness to announce bonuses
    kontra_aggression: float = 0.0  # Willingness to kontra/re/sub

    # --- Exploration traits ---
    temperature: float = 1.0        # Softmax temperature (0.3 to 3.0)
    explore_decay: float = 0.995    # Per-session multiplicative decay of explore_rate
    explore_floor: float = 0.02     # Minimum exploration rate
    ...
    def to_genes(self) -> list[float]:
        """Flatten to a gene list for DEAP."""
        return [
            self.bid_aggression,
    ...
    def from_genes(genes: list[float]) -> BehavioralProfile:
        """Reconstruct from a DEAP gene list."""
        return BehavioralProfile(
...
# Gene bounds for DEAP mutation clipping
...
def apply_behavioral_bias(
    ...
    if decision_type == DecisionType.BID:
        # Boost all non-pass bids
        for idx in _NONPASS_BID_INDICES:
            bias[..., idx] += profile.bid_aggression * _BIAS_SCALE
        # Extra boost for solo contracts
        for idx in _SOLO_BID_INDICES:
            bias[..., idx] += profile.solo_propensity * _BIAS_SCALE * 0.7

    elif decision_type == DecisionType.CARD_PLAY:
        # Bias toward/against playing taroks
        for idx in _TAROK_CARD_INDICES:
            bias[..., idx] += profile.trump_eagerness * _BIAS_SCALE * 0.5
        # When defending, bias toward low-value (cautious) play:
        # defensive_caution > 0 means prefer low-index (low-value) cards
        if is_defender and abs(profile.defensive_caution) > 0.01:
            # Gradient bias: lower-index suit cards get a boost, higher get penalty
            for idx in range(22, CARD_ACTION_SIZE):  # suit cards only
                # Normalize position in suit cards range [22..53] to [-1, 1]
        ...
        # No strong behavioral bias for talon — it's mostly strategic
        pass

    elif decision_type == DecisionType.KING_CALL:
        # No behavioral bias — king calling is purely hand-dependent
        pass

    # Only apply bias to legal actions
