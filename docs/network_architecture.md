...
**Input Projection:**
1. `Linear(639, 256)` → `LayerNorm(256)` → `ReLU`
...
- 54 card tokens → `Linear(10, 64)` → `MultiheadAttention(64, 4 heads)` → `LayerNorm` → `MeanPool` → `Linear(64, 64)`
...
1. `Linear(801, 256)` → `LayerNorm(256)` → `ReLU`
2. `Linear(256, 256)` → `LayerNorm(256)` → `ReLU`
